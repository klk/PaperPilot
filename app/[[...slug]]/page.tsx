...
import { capabilityForTool, capabilityLabel, categories, mergeToolPublishState, toolMap, uniqueTools, type Tool } from "../../lib/tools";
...
function ToolCard({ item, locale }: { item: Tool; locale: Locale }) {
  const capability = capabilityForTool(item);
  return <Link className="tool-card" href={`/${item.slug}`}>
    <div className="tool-card-top"><ToolIcon slug={item.slug} /><span className={`capability-badge capability-${capability}`}>{capabilityLabel(capability, locale)}</span></div>
